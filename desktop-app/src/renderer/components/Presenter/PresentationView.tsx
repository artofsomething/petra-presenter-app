...
import RichText, { hasInlineMarkup } from '../Editor/RichText';
...
const PresentationView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    socket.on('presentation-updated', (data: any) => {
       addDebug(
        `presentation-updated: ${data.presentation?.slides?.length ?? 0} slides`
      );
      if (data.presentation) {
        setPresentation(data.presentation);
      }
      // ✅ ADD: also update slide index if provided
      if (data.currentSlideIndex != null) {
        setCurrentSlideIndex(data.currentSlideIndex);
        addDebug(`presentation-updated: index → ${data.currentSlideIndex}`);
      }
    });
...
const PresentationText: React.FC<{ element: SlideElement }> = ({ element }) => {
  ...
  // ✅ Pass through typed functions — no inline IIFE needed
  ...
  const underlineLines = element.underline
    ? getRenderedLines({
        ...
        // Already inside scaled Layer — no extra transform needed
        stageScaleX:   1,
        stageScaleY:   1,
        stageX:        0,
        stageY:        0,
  ...
  return (
    <>
      ...
      {/* ── Underlines — works for both rich and plain paths ─────────────── */}
