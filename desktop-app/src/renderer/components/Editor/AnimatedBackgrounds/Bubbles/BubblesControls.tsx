...
interface BubblesControlsProps {
  current:  AnimatedBackground;
  onChange: (updated: Partial<AnimatedBackground>) => void;
}

const BubblesControls: React.FC<BubblesControlsProps> = ({ current, onChange }) => {
  ...
  return (
    <div style={{
      ...
      <ColorRow label="Bubble 1" value={c1} onChange={(v) => onChange({ color1: v })} />
      <ColorRow label="Bubble 2" value={c2} onChange={(v) => onChange({ color2: v })} />
      <ColorRow label="Bubble 3" value={c3} onChange={(v) => onChange({ color3: v })} />

    </div>
  );
};

export default BubblesControls;